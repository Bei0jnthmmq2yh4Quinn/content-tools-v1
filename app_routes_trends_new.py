...
XHS_TREND_CACHE = Path('/tmp/mobang_xhs_trends_cache.json')
XHS_TIMEOUT = 20.0
...
def _load_xhs_cache() -> List[Dict[str, Any]]:
    try:
        if not XHS_TREND_CACHE.exists():
            return []
        payload = json.loads(XHS_TREND_CACHE.read_text())
        return payload.get('data') or []
    except Exception:
        return []


def _save_xhs_cache(items: List[Dict[str, Any]]) -> None:
    try:
        XHS_TREND_CACHE.write_text(json.dumps({'ts': int(time.time()), 'data': items}, ensure_ascii=False))
    except Exception:
        pass
...
async def _fetch_xiaohongshu_trends() -> List[Dict[str, Any]]:
    ...
    for headers in headers_list:
        try:
            async with httpx.AsyncClient(timeout=XHS_TIMEOUT, follow_redirects=True) as client:
                resp = await client.get(XHS_EXPLORE_URL, headers=headers)
                resp.raise_for_status()
                html = resp.text
            ...
            for item in feeds:
                ...
                if len(results) >= 20:
                    break
            if results:
                _save_xhs_cache(results)
                return results
        except Exception as e:
            last_error = e
            continue

    cached = _load_xhs_cache()
    if cached:
        return cached
    raise ValueError(f'未能获取小红书热门: {last_error}')
